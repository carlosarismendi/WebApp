<template>
  <div class="container py-3 sign-in">
    <div class="text-center shadow-lg p-3 px-4 rounded">
      <b-overlay :show="showOverlay" rounded="sm" no-wrap opacity="0.6"></b-overlay>
          <b-alert
            variant="danger"
            dismissible
            :show="showError"
            @dismissed="resetError()"
          >
            {{errorMessage}}
          </b-alert>
      <h1>Sign in</h1>
      <ValidationObserver ref="observer" v-slot="{ invalid }" v-show="!indRegisterEnd">
        <form id="sign-in-Form" @submit.prevent="signIn" class="row">

          <ValidationProvider name="user" :rules="{ required: true, max: 250 }" v-slot="validationContext" class="col-12">
            <b-form-group id="user-group">
              <label for="user">User</label>
              <b-form-input
                id="user"
                name="user"
                type="text"
                :autofocus="true"
                tabindex="1"
                v-model="user.user"
                :state="getValidationState(validationContext)"
                :maxlength="250"
              ></b-form-input>
            </b-form-group>
          </ValidationProvider>

          <ValidationProvider name="password" :rules="{ required: true, min: 8, max: 250 }" v-slot="validationContext" class="col-12">
            <b-form-group id="password-group">
              <label for="password">Password</label>
              <b-form-input
                id="password"
                name="password"
                :type="indPasswordType"
                :autofocus="true"
                tabindex="3"
                v-model="user.password"
                :state="getValidationState(validationContext)"
                :maxlength="250"
                style="background-image: none !important;"
              ></b-form-input>
              <div class="btn btn-eye" @click="togglePasswordVisibility">
                <i :class="indPasswordIcon"></i>
              </div>
            </b-form-group>
          </ValidationProvider>

          <div class="col-12">
            <button :disabled="invalid" type="submit" class="btn btn-primary w-50 mt-2">Log in</button>
          </div>
        </form>
      </ValidationObserver>

      <div v-show="indRegisterEnd" class="text-left">
        <p>You have been registered succesfully.</p>
        <p>Please check your email inbox and confirm your account by using the provided link.</p>
      </div>
    </div>
  </div>
</template>

<script>
import UTILS from '../../utils'

export default {
  name: 'SignIn',
  data () {
    return {
      newImage: null,
      indPasswordType: 'password',
      indPasswordIcon: 'far fa-eye-slash',
      indRegisterEnd: false,
      user: {
        user: '',
        password: ''
      },
      showOverlay: false,
      showError: false,
      errorMessage: ''
    }
  },
  methods: {
    getValidationState ({ dirty, validated, valid = null }) {
      return dirty || validated ? valid : null
    },
    resetError () {
      this.showError = false
      this.errorMessage = null
    },
    togglePasswordVisibility () {
      if (this.indPasswordType === 'text') {
        this.indPasswordType = 'password'
        this.indPasswordIcon = 'far fa-eye-slash'
      } else {
        this.indPasswordType = 'text'
        this.indPasswordIcon = 'fal fa-eye'
      }
    },
    signIn () {
      this.showOverlay = true
      this.axios.post('login', this.user)
        .then(res => {
          if (res.data.status < 0) {
            this.errorMessage = res.data.error
          } else {
            UTILS.user.setLogin(res.data.data.jwtToken, res.data.data.idUser)

            UTILS.user.setProfileImage(res.data.data.profileImage)
            UTILS.user.setNickname(res.data.data.nickname)
            UTILS.user.setName(res.data.data.name)

            this.$router.push({ name: 'app.home' })
          }
        })
        .catch(e => {
          console.log(e)
          this.errorMessage = e.message
        })
        .finally(() => {
          this.showOverlay = false
          this.showError = true
        })
    }
  },
  created() {
    if (UTILS.user && UTILS.user.isAuthenticated()) {
      this.$router.push({ name: 'app.home' })
    }
  }
}
</script>

<style scoped>
.btn-eye {
  position: relative!important;
  float: right;
  font-size: 1em;
  margin-top: -38px;
  /* margin-right: -89%; */
}

label {
  float: left;
}

@media (max-width: 600px) {
  .sign-in {
    margin-top: 20px;
    width: 90%;
  }
}

@media (min-width: 601px) and (max-width: 750px) {
  .sign-in {
    margin-top: 30px;
    width: 65%;
  }
}

@media (min-width: 751px) and (max-width: 900px) {
  .sign-in {
    margin-top: 40px;
    width: 50%;
  }
}

@media (min-width: 901px) and (max-width: 1200px) {
  .sign-in {
    margin-top: 50px;
    width: 40%;
  }
}

@media (min-width: 1201px) {
  .sign-in {
    margin-top: 60px;
    width: 25%;
  }
}
</style>