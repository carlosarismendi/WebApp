...
IF  EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[usp_followsListGet]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
    DROP PROCEDURE [dbo].usp_followsListGet
GO

create procedure usp_followsListGet(
	@p_json		nvarchar(max),
	@p_idUser	int
)
as
begin
	begin try
		...
		select	@p_idUserFollower	= JSON_VALUE(@p_json, '$.idUserFollower'),
		...
		begin
			insert into #tmpFollows (idFollow, idUserFollowed, nickname, name, profileImage)
			select
					f.id				as idFollow,
					f.idUserFollowed	as idUserFollowed,
					u.nickname			as nickname,
					u.name				as name,
					u.profileImage		as profileImage
			from	[follow] f with(nolock)
			inner	join [user] u with(nolock)
				on	u.id = f.idUserFollowed
					and u.indActive = 1
			where	f.id <= @p_prev
				and f.idUserFollower = @p_idUserFollower
		...
		select 	top (@p_limit) idFollow, idUserFollowed, nickname, name, profileImage
		into	#tmpResult
		from	#tmpFollows
		order	by
